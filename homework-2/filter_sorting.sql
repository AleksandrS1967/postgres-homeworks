...
SELECT AVG(shipped_date - order_date) as days_average
FROM orders
...
SELECT MIN(unit_price + discontinued) as min_price,
MAX(discontinued + unit_price) as max_price
FROM products
...
SELECT MIN(unit_price + discontinued),
MAX(unit_price + discontinued)
FROM products
WHERE discontinued <> 1
